...
std::expected<std::size_t, Error> bedrock_varint_payload_size(std::span<const std::byte> bytes, std::size_t offset,
							      TagId tag)
{
...
std::expected<ChildRecord, Error> bedrock_varint_read_child(std::span<const std::byte> bytes, std::size_t offset)
{
	Cursor c{bytes, offset};
	...
	auto name_len = read_uvarint32(c);
	if (!name_len) {
