...
[[nodiscard]] std::expected<std::size_t, Error> skip_payload(Cursor &c, TagId tag)
{
	...
	case TagId::Compound: {
		while (true) {
			...
			if (auto r = skip_payload(c, *child); !r) {
				return r;
			}
		}
	}
...
std::expected<std::size_t, Error> java_payload_size(std::span<const std::byte> bytes, std::size_t offset, TagId tag)
{
	Cursor c{bytes, offset};
	return skip_payload(c, tag);
}

std::expected<ChildRecord, Error> java_read_child(std::span<const std::byte> bytes, std::size_t offset)
{
	Cursor c{bytes, offset};
	...
	auto name_len = c.read_int_be<std::uint16_t>();
	if (!name_len) {
