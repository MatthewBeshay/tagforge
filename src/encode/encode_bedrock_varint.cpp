...
[[nodiscard]] std::expected<void, Error> write_string(std::vector<std::byte> &out, std::string_view s)
{
	...
	write_uvarint32(out, static_cast<std::uint32_t>(s.size()));
	const std::byte *src = reinterpret_cast<const std::byte *>(s.data());
	out.insert(out.end(), src, src + s.size());
	return {};
}

[[nodiscard]] std::expected<void, Error> write_count(std::vector<std::byte> &out, std::size_t n)
{
	if (n > static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max())) {
		return std::unexpected(make_error(ErrorCode::LengthOverflow));
	}
	write_zigzag32(out, static_cast<std::int32_t>(n));
	return {};
}
...
[[nodiscard]] std::expected<void, Error> write_payload(std::vector<std::byte> &out, const Value &v)
{
	...
	case TagId::Short:
		...
		return {};
	case TagId::Int:
		write_zigzag32(out, std::get<std::int32_t>(v.v));
		return {};
	case TagId::Long:
		write_zigzag64(out, std::get<std::int64_t>(v.v));
		return {};
	case TagId::Float:
	...
	case TagId::ByteArray: {
		...
		if (auto e = write_count(out, arr.size()); !e) {
			return e;
		}
	...
	case TagId::List: {
		...
		if (auto e = write_count(out, list.size()); !e) {
			return e;
		}
...
std::expected<void, Error> encode_bedrock_varint(const NamedValue &nv, std::vector<std::byte> &out)
{
